...
focusstack (const Arg *arg)
{
  ...
  if (!selmon->sel
      || (selmon->sel->isfullscreen && !selmon->sel->fakefullscreen))
    return;
  if (arg->i > 0)
    {
      ...
      for (i = 0; i < LENGTH (keychords); i++)
        ...
        if (keychords[i]->keys[currentkey].keysym == syms[(k - start) * skip])
          for (c = 0; c < LENGTH (modifiers); c++)
            XGrabKey (dpy, k,
                      keychords[i]->keys[currentkey].mod | modifiers[c], root,
                      True, GrabModeAsync, GrabModeAsync);
    if (currentkey > 0)
      XGrabKey (dpy, XKeysymToKeycode (dpy, XK_Escape), AnyModifier, root,
                True, GrabModeAsync, GrabModeAsync);
    XFree (syms);
  }
}
...
placemouse (const Arg *arg)
{
  ...
  if (arg->i == 2) // warp cursor to client center
    XWarpPointer (dpy, None, c->win, 0, 0, 0, 0, WIDTH (c) / 2,
                  HEIGHT (c) / 2);
  ...
  do
    {
      ...
      switch (ev.type)
        ...
        case MotionNotify:
          ...
          if (arg->i == 1)
            { // tiled position is relative to the client window center point
              px = nx + wa.width / 2;
              py = ny + wa.height / 2;
            }
          else
            { // tiled position is relative to the mouse cursor
              px = ev.xmotion.x;
              py = ev.xmotion.y;
            }
...
setfullscreen (Client *c, int fullscreen)
{
   ...
   * we are in said state, but the client itself disables fullscreen (3) then
   * we let the client go out of fullscreen while keeping fake fullscreen
   * enabled (as otherwise there will be a mismatch between the client and the
   * window manager's perception of the client's fullscreen state). */
   ...
   * window manager that it is going into fullscreen after receiving the above
   * signal. This has the side effect of this function (setfullscreen)
   * sometimes being called twice when toggling fullscreen on and off via the
   * window manager as opposed to the application itself. To protect against
   * obscure issues where the client settings are stored or restored when they
   * are not supposed to we add an additional bit-lock on the old state so that
    ...
    if (i < m->nmaster)
    ...
    else
      {
        ...
        if ((i - m->nmaster) % 2)
          {
            h = (m->wh - ety) / ((1 + n - i) / 2);
            resize (c, m->wx, m->wy + ety, tw - (2 * c->bw), h - (2 * c->bw),
                    0);
            ety += HEIGHT (c);
          }
        else
